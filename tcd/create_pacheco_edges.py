...
def process_tweets(input_file, output, min_num_tweets, min_num_hashtags):
    ...
    #
    # create hashtag sequences that are the ordered hashtags for each (account, date) pair
    #
    ...
    # the resulting list is ordered by the created_at column
    def ordered_hashtags(d):
        hts = []
        for i, row in d.iterrows():
            hts.append((row["created_at"], row["hashtags"]))

        hts_sorted = [ht for _, ht in sorted(hts, key=lambda x: x[0])]

        hts_flattened = [ht for sublist in hts_sorted for ht in sublist]

        return pd.Series(
            {
                "hashtags": hts_flattened,
            }
        )

    df = pd.DataFrame(
        df.groupby(["author_id", "date"], as_index=False).apply(
            ordered_hashtags, include_groups=False
        )
    )
    ...
    # create a string representation of the hashtags
